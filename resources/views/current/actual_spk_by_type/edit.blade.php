...
@section('title', 'Edit Actual SPK By Type')
            ...
            <form id="editForm" action="{{ route('current.actual-spk-by-type.update', $actualSpkByType->id) }}"
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="mobil_type"
                            ...
                            @foreach (['NEW CARRY', 'APV', 'ERTIGA', 'XL7', 'SPRESO', 'BALENO', 'GRAND VITARA', 'JIMNY 3D', 'JIMNY 5D', 'FRONX'] as $type)
                                <option value="{{ $type }}"
                                    {{ $actualSpkByType->mobil_type == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun" value="{{ $actualSpkByType->tahun }}"
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    @foreach ($months as $m)
                        <div style="background: #f7fafc; padding: 10px; border-radius: 12px; border: 1px solid #edf2f7;">
                            ...
                            <input type="number" name="{{ $m }}" value="{{ $actualSpkByType->$m }}"
    ...
    <script>
        ...
        function confirmUpdate() {
            Swal.fire({
                ...
                text: "Data aktual SPK ini akan diperbarui sesuai input Anda.",
                icon: 'question',

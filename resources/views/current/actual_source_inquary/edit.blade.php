            ...
            <form id="editForm" action="{{ route('current.actual-source-inquary.update', $actualSourceInquary->id) }}"
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="source_inquary"
                            ...
                            @foreach (['Call In (dari Iklan)', 'Canvasing', 'Data Base', 'Digital Hyperlocal', 'Digital Non Hyperlocal', 'Exibition', 'Media Digital', 'Media Elektronik', 'Mediator', 'Referensi', 'Referensi Customer', 'Showroom Activity', 'Showroom Walk In', 'Website Dealer'] as $src)
                                <option value="{{ $src }}"
                                    {{ $actualSourceInquary->source_inquary == $src ? 'selected' : '' }}>
                                    {{ $src }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun" value="{{ $actualSourceInquary->tahun }}"
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    @foreach ($months as $m)
                        <div style="background: #f7fafc; padding: 10px; border-radius: 12px; border: 1px solid #edf2f7;">
                            ...
                            <input type="number" name="{{ $m }}" value="{{ $actualSourceInquary->$m }}"
    ...
    <script>
        ...
        function confirmUpdate() {
            Swal.fire({
                ...
                text: "Data actual source inquiry akan diperbarui.",
                icon: 'question',

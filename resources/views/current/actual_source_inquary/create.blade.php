...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #cbd5e0; font-size: 14px;">Input data baru aktual berdasarkan sumber inquiry</p>
        </div>

        <div
            style="background: white; width: 100%; max-width: 700px; padding: 35px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            <form id="sourceInquiryForm" action="{{ route('current.actual-source-inquary.store') }}" method="POST">
                @csrf
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="source_inquary" id="source_inquary"
    ...
    <script>
        function confirmSubmit() {
            const sourceInquiry = document.getElementById('source_inquary').value;
            const tahun = document.getElementById('tahun').value;

            // Validasi Input
            if (!sourceInquiry || !tahun) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Source Inquiry dan Tahun wajib diisi!',
            ...
            Swal.fire({
                title: 'Simpan Data?',
                text: "Pastikan data bulanan yang dimasukkan sudah benar.",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('sourceInquiryForm').submit();
                }
            });
        }

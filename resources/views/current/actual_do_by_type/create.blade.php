...
@section('title', 'Tambah Actual DO By Type')
...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #cbd5e0; font-size: 14px;">Input data baru aktual delivery order tipe kendaraan</p>
        </div>

        <div
            style="background: white; width: 100%; max-width: 700px; padding: 35px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            <form id="actualDoForm" action="{{ route('current.actual-do-by-type.store') }}" method="POST">
                @csrf
    ...
    <script>
        function confirmSubmit() {
            ...
            // Validasi Sederhana agar tidak kena Error Column 'tahun' cannot be null lagi
            if (!mobilType || !tahun) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('actualDoForm').submit();
                }
            });
        }

        // Tampilkan alert sukses jika ada flash message dari controller
        @if (session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Berhasil!',

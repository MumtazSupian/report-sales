...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #cbd5e0; font-size: 14px;">Perbarui data aktual jumlah tenaga penjual berdasarkan grading</p>
        </div>
            ...
            <form id="editForm" action="{{ route('current.actual-salesforces.update', $actualSalesforce->id) }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="grading" id="grading"
                            ...
                            @foreach (['PLATINUM', 'GOLD', 'SILVER', 'TRAINEE', 'FREELANCE'] as $g)
                                <option value="{{ $g }}" {{ $actualSalesforce->grading == $g ? 'selected' : '' }}>
                                    {{ $g }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun" id="tahun" value="{{ $actualSalesforce->tahun }}"
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    @foreach ($months as $m)
                        <div style="background: #f7fafc; padding: 10px; border-radius: 12px; border: 1px solid #edf2f7;">
                            ...
                            <input type="number" name="{{ $m }}" value="{{ $actualSalesforce->$m }}" min="0"
    ...
    <script>
        function confirmUpdate() {
            Swal.fire({
                ...
                text: "Data grading salesforce akan diperbarui di sistem.",
                icon: 'question',

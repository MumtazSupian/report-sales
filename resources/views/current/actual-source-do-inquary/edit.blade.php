...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #cbd5e0; font-size: 14px;">Perbarui data aktual delivery order berdasarkan sumber inquiry</p>
        </div>

        <div
            style="background: white; width: 100%; max-width: 700px; padding: 35px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            <form id="editForm" action="{{ route('current.actual-source-do-inquary.update', $actualSourceDoInquary->id) }}"
                method="POST">
                @csrf
                @method('PUT')
    ...
    <script>
        function confirmUpdate() {
            Swal.fire({
                ...
                text: "Pastikan data aktual DO sudah sesuai sebelum disimpan.",
                icon: 'question',

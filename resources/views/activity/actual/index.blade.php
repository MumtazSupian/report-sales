...
@section('content')
    <div style="padding: 20px;">
        <h2
            style="text-align:center; font-weight:800; color:#fff; letter-spacing:0.5px; text-transform:uppercase; margin-bottom:20px;">
            MARKETING ACTIVITY ACTUAL
        </h2>
        ...
        <div style="background:#fff; padding:15px; border-radius:14px; box-shadow:0 6px 20px rgba(0,0,0,0.15);">
            <table
                style="width:100%; border-collapse:collapse; font-family:'Segoe UI',sans-serif; font-size:10px; text-align:center; border: 1px solid #000;">
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr style="background:#fff; border-bottom: 1px solid #000;">
                            ...
                            <td style="border: 1px solid #000;">{{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}
                            </td>
                            ...
                            <td style="border: 1px solid #000; text-align: left; padding: 5px;">{{ $item->keterangan }}
                            </td>
                            <td style="border: 1px solid #000; white-space:nowrap; padding: 5px;">
                                <a href="{{ route('activity.actual.edit', $item->id) }}"
                                ...
                                <form action="{{ route('activity.actual.destroy', $item->id) }}" method="POST"
                                    style="display:inline;" id="delete-form-{{ $item->id }}">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $item->id }}')"
                                        style="background:#fff5f5; color:#e53e3e; border:1px solid #feb2b2; padding:3px 8px; border-radius:4px; font-size: 10px; cursor:pointer; font-weight:800; text-transform: uppercase;">HAPUS</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
                ...
                <tfoot style="background:#0d47a1; color:white; font-weight:bold;">
                    <tr style="border-top: 2px solid #000;">
                        ...
                        <td style="border: 1px solid #000;">Rp{{ number_format($data->sum('total_cost'), 0, ',', '.') }}
                        </td>
                        <td style="border: 1px solid #000;">@php $tp = $data->sum('actual_p'); @endphp
                            Rp{{ $tp > 0 ? number_format($data->sum('total_cost') / $tp, 0, ',', '.') : '0' }}</td>
                        <td style="border: 1px solid #000;">@php $ts = $data->sum('actual_spk'); @endphp
                            Rp{{ $ts > 0 ? number_format($data->sum('total_cost') / $ts, 0, ',', '.') : '0' }}</td>

...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #cbd5e0; font-size: 14px;">Input rencana target penjualan baru berdasarkan sumber inquiry</p>
        </div>
            ...
            <form id="targetSoiForm" action="{{ route('rka.target-do-by-soi.store') }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="source_inquiry" id="source_inquiry" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px; color: #4a5568;">
    ...
    <script>
        function confirmSubmit() {
            const soi = document.getElementById('source_inquiry').value;
            if (!soi) { Swal.fire('Error', 'Pilih Source Inquiry!', 'error'); return; }
            Swal.fire({
                title: 'Simpan Target?',
                text: "Simpan target untuk " + soi + "?",
                icon: 'question',

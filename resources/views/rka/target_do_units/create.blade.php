...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div
            ...
            <form id="targetUnitForm" action="{{ route('rka.target-do-units.store') }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        <label
                            ...
                            Type</label>
                        <select name="mobil_type" id="mobil_type" required
                ...
                <label
                    style="display: block; font-weight: 800; color: #3182ce; margin-bottom: 15px; font-size: 14px; text-transform: uppercase; text-align: center;">Target
                    Per Bulan</label>
    ...
    <script>
        function confirmSubmit() {
            ...
            if (!type) {
                Swal.fire('Error', 'Pilih Tipe Mobil!', 'error');
                return;
            }
            Swal.fire({
                title: 'Simpan Target?',
                ...
                if (result.isConfirmed) document.getElementById('targetUnitForm').submit();
            });
        }
    </script>
@endsection

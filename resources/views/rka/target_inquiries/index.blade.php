...
@section('content')
    <div style="padding: 20px; max-width: 1300px; margin: 0 auto;">
        ...
        <p style="text-align:center; color: #cbd5e0; margin-bottom:20px; font-size: 14px;">Manajemen target prospek masuk per sumber inquiry</p>
        ...
        <div style="background:#fff; padding:15px; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.3); overflow-x:auto;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead style="background:#f1f5f9; color:#0d47a1;">
                    <tr style="border-bottom:2px solid #cbd5e0;">
                        <th style="border:1px solid #999; padding: 12px; text-align: left;">SOURCE INQUIRY</th>
                        ...
                        @endforeach
                        <th style="border:1px solid #999; background:#e2e8f0; width: 65px;">TOTAL</th>
                        <th style="border:1px solid #999; width: 110px;">AKSI</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data as $row)
                        <tr style="background:{{ $loop->iteration % 2 == 0 ? '#f8fafc' : '#ffffff' }}; border-bottom:1px solid #ccc;">
                            <td style="border:1px solid #bbb; font-weight:600; text-align:left; padding-left:12px;">{{ $row->source_inquiry }}</td>
                            ...
                            @endforeach
                            <td style="border:1px solid #bbb; font-weight: 800; background:#f1f5f9;">{{ number_format($row->total, 0, ',', '.') }}</td>
                ...
                <tfoot style="background:#0d47a1; color:white; font-weight:bold;">
                    <tr>
                        <td colspan="3" style="border:1px solid #999; padding: 10px; text-align: center;">GRAND TOTAL</td>
                        @foreach ($months as $m)

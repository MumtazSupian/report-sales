...
<div style="padding: 20px 40px; font-family: 'Segoe UI', sans-serif;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <div>
            <h2 style="color: white; margin: 0; font-weight: 700; letter-spacing: 1px;">TARGET INQUIRIES</h2>
            <p style="color: #cbd5e0; margin: 5px 0 0 0; font-size: 14px;">Manajemen target prospek masuk (prospecting) per sumber inquiry</p>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; min-width: 1100px;">
            <thead>
                <tr style="background: #2d3748; color: white;">
                    <th style="padding: 15px; text-align: left; border-radius: 10px 0 0 0;">SOURCE INQUIRY</th>
            ...
            <tbody>
                @foreach($data as $row)
                    <tr style="border-bottom: 1px solid #edf2f7; transition: 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                        <td style="padding: 12px 15px; font-weight: 700; color: #2d3748;">{{ $row->source_inquiry }}</td>
                        ...
                        <td style="padding: 12px 15px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                ...
                                <form action="{{ route('rka.target-inquiries.destroy', $row->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
